...
<head>
  ...
  <div class ='Header'>
    <?php echo $HeaderInfo; ?>
    <div class ='Header_ScreenName'><p>QRコード作成画面</p></div>
  </div>
</head>
...
<?php
  ...
  if (!empty($_POST["Request"])) {
    ...
    if ($Request <= $Quantity){       
      ...
      $alert = "<h2>選択した日付では既に[" . $Quantity. "]個のQRコードが作成されています。</h2>";
      ...
      foreach ($Key_Code_Table as $value) {
        ...
        // 挿入する文字列の色(白)
        $color = imagecolorallocate($Create_img, 255, 255, 255);
        ...
        //パスワード挿入--開始--
         ...
         // 挿入位置
         $x = imagesx($Create_img) * (1/3); // 左からの座標(ピクセル)
        ...
        imageLayerEffect($Qr_img, IMG_EFFECT_ALPHABLEND); // 合成する際、透過を考慮する
        imagecopy($Create_img, $Qr_img, 65, 95, 0, 0, $sx, $sy); // 合成する
